...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Draw cursor (semi-transparent so text is visible)
    float sdfCursor = getSdfRectangle(vu, currentCursor.xy - (currentCursor.zw * offsetFactor), currentCursor.zw * 0.5);
    float cursorAlpha = (1.0 - smoothstep(0.0, 0.01, sdfCursor)) * 0.3; // 30% opacity
    newColor = mix(newColor, CURRENT_CURSOR_COLOR, cursorAlpha);
